...
namespace TechFit
{
    public partial class frmCadastroLucro : Form
    {
        acesssoLucroDespesa acessarLD = new acesssoLucroDespesa();

        public frmCadastroLucro()
        {
            InitializeComponent();
        }
        ...
        private void lblBtnEntrar_Click(object sender, EventArgs e)
        {
            if (txtDescricao.Text == string.Empty || txtValor.Text == string.Empty)
            {
                ...
                acessarLD.inserir(txtDescricao.Text,"Lucro",acessoDadosLogado.Nome, acessoDadosLogado.Login, txtValor.Text);
                MessageBox.Show("Lucro cadastrado com sucesso!", "Lucro Cadastrado", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtDescricao.Clear();
                txtValor.Clear();
